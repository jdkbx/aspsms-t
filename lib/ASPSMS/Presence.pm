...
if ($type eq 'subscribe') {
  ...
  aspsmst_log('debug',"InPresence(): type=$type to=$to number=$number");

  if (($number !~ /^\+[0-9]{3,50}$/) && ($to =~ /\@/)) {
  ...
  if (($to eq "$ASPSMS::config::service_name/registered") or
     ($to eq "$ASPSMS::config::service_name")) {
